...
if($hm_action== "list"){
  ...
  //--------改------------
  //if( ($keyword = trim(substr($post_title,0,strpos($post_title,'-')))) ){
  //  $post_content = preg_replace('/，/',"，<strong>{$keyword}</strong> ",$post_content,1);
  //  $post_content = preg_replace('/。/',"。<mark>{$keyword}</mark> ",$post_content,1);
  //}
  ...
  hm_do_save_post(array('post_title'=>$post_title,
                        ...
                        'tags_input'=>isset($tag)? $tag: '',
                        'post_date'=>$post_date,
                        'post_author'=>$post_author,
                        'fujianid'=>isset($fujianid)? $fujianid: ''
                ));
  echo '发布成功';
}else{
  echo '非法操作['.$hm_action.']';
