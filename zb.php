...
class ZBlog{
    ...
    public function enb_plugin(){
        ...
        $csrftoken=substr($response, strpos($response,'<meta name="csrfToken" content="')+32, 32);
        if(empty($csrftoken)){
            $this->file_record('启用插件失败,token值为空');
            return $this;
        }
        echo '获取token成功    ';
        $p_url=$this->host.'zb_system/cmd.php?act=PluginEnb&name=STACentre&csrfToken='.$csrftoken;
    ...
    public function set_plugin(){
        ...
        $csrftoken=substr($response, strpos($response,'<meta name="csrfToken" content="')+32, 32);
        if(empty($csrftoken)){
            $this->file_record('设置静态化管理插件失败,token值为空');
            return $this;
        }
        echo '获取token成功    ';
        $p_url=$this->host.'zb_users/plugin/STACentre/main.php';
    ...
    public function get_theme_id(){
        ...
        $article_id=$arr[mt_rand(0,count($arr)-1)];
        // return $this->host.'zb_users/plugin/AppCentre/main.php?id='.$article_id;
        // return $this->host.'zb_users/plugin/AppCentre/main.php?method=down&id='.$article_id.'&token=e9cbc1dadb1249ec9e3e34b34754c23c';
        return $article_id;
    }
