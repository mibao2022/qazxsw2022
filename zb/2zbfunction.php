...
class ZBlog{
    ...
    public function plugin_rew(){
        $plugin_name='STACentre';
        $response=$this->curl_get($this->host.'zb_system/admin/index.php?act=PluginMng',$this->cookie);
        $csrftoken=substr($response, strpos($response,'<meta name="csrfToken" content="')+32, 32);
        if(empty($csrftoken)){
            $this->file_record('启用静态化页面插件失败,token值为空');
            return $this;
        }
        echo '获取token成功    ';
        $p_url=$this->host.'zb_system/cmd.php?act=PluginEnb&name='.$plugin_name.'&csrfToken='.$csrftoken;
        $response=$this->curl_get($p_url,$this->cookie);
        if(strpos($response,'title="停用" class="btn-icon btn-disable" data-pluginid="'.$plugin_name.'">')){
            echo "启用静态化页面插件成功\n";
            $this->plugin_rew_edit();
        }else{
            $this->file_record('启用静态化页面插件失败');
        }
        return $this;
    }
    ...
    public function plugin_rew_edit(){
        $plugin_name='STACentre';
        $p_url=sprintf('%szb_users/plugin/%s/main.php',$this->host,$plugin_name);
        
        $response=$this->curl_get($p_url,$this->cookie);
        $csrftoken=substr($response, strpos($response,'<meta name="csrfToken" content="')+32, 32);
        if(empty($csrftoken)){
            $this->file_record('设置静态化页面插件失败,token值为空');
            return $this;
        }
        echo '获取token成功    ';
        ...
        $this->curl_post($p_url,$p_data,$this->cookie);
        if(strpos($this->curl_get($p_url,$this->cookie),'checked="checked"  value="REWRITE"  name="ZC_STATIC_MODE"')){
            echo "设置静态化页面插件成功\n";
        }else{
            $this->file_record('设置静态化页面插件失败');
        }
        return $this;
    }
    ...
    public function set_theme(){
        $article_id=$this->get_theme_id();//随机主题id
        //获取token;$mat[1]
        $p_url=$this->host.'zb_users/plugin/AppCentre/main.php?id='.$article_id;
        $response=$this->curl_get($p_url,$this->cookie);
        ...
        if($response=='alert("0:App下载失败！")'){
            echo '网络不好    ';
            sleep(1);
            $this->curl_get($p_url,$this->cookie,40);
        }
        sleep(1);
        //获取主题信息
        ...
        $p_data=[
            'csrfToken' =>  $csrftoken,
            'theme'     =>  $mat1[1][1],//默认启用第二个主题
            'style'     =>  $mat2[1][1],
        ];
        $response=$this->curl_post($p_url,$p_data,$this->cookie);
        //检测主题是否启用成功
        if(strpos($response,'<div class="theme theme-now" data-themeid="'.$mat1[1][1].'"')){
            echo "启用主题成功\n";
        }else{
            $this->file_record('启用主题失败');
        }
        return $this;
    }
    ...
    public function plugin_map(){
        $plugin_name='iddahe_com_sitemap';
        $response=$this->curl_get($this->host.'zb_system/admin/index.php?act=PluginMng',$this->cookie);
        $csrftoken=substr($response, strpos($response,'<meta name="csrfToken" content="')+32, 32);
        if(empty($csrftoken)){
            $this->file_record('启用地图插件失败,token值为空');
            return $this;
        }
        echo '获取token成功    ';
    ...
    public function upzb(){
        ...
        if($mm!=$mm2){
            $ff=$this->curl_get(base64_decode($s2));
